...
status_t configureInterrupt(uint32_t gpioPort, uint8_t pinNumber){
	...
	/* we need activate syscfg to be able choose GPIO port for each EXTI interrupt line */
     ...
     /* PUPDR 01: Pull-up */
     GPIOA->PUPDR|=((0x1UL<<GPIO_PUPDR_PUPD0_Pos)|(0x1UL<<GPIO_PUPDR_PUPD0_Pos));

     /* route pins with EXTIs */
     /* 0000: PA[0] pin */
     SYSCFG->EXTICR&=~(0b1111<<SYSCFG_EXTICR1_EXTI0_Pos);
     /* 0000: PA[1] pin */
     SYSCFG->EXTICR&=~(0b1111<<SYSCFG_EXTICR1_EXTI1_Pos);
     ...
     EXTI->IMR1|=((1UL<<EXTI_IMR1_IM0_Pos)|(1UL<<EXTI_IMR1_IM0_Pos));
